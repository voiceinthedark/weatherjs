...
class HourUI{
  ...
  #renderHourCard(hour){
    ...
    hourTime.textContent = format(parse(hour.datetime, "HH:mm:ss", new Date()), "H a"); 

    /** HOUR CARD **/
    const hourCard = this.#uimanager.addElement('div', hourFrame, 'hour-card')
    /** HOUR HEADER **/
    const hourHeader = this.#uimanager.addElement('div', hourCard, 'hour-header')
    ...
    const hourTemp = this.#uimanager.addElement('span', hourHeader, 'hour-temp')
    hourTemp.textContent = ''+hour.temp;

    const hourConditions = this.#uimanager.addElement('span', hourCard, 'hour-conditions');
